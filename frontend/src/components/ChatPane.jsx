 ...
 * Photo fix: uses label+htmlFor instead of programmatic click, plus canvas compression
 */
import { useState } from 'react';
import { Avatar } from './Sidebar';
import ProfileViewer from './ProfileViewer';

/** Compress to JPEG ≤900px so image fits in the socket payload (~60-80KB) */
...
export default function ChatPane({
    ...
    // ── Photo pick: label+htmlFor is the most reliable cross-browser approach ──
    const handleImagePick = async (e) => {
        ...
        reader.onload = async (ev) => {
            try {
                ...
                onSendImage(ev.target.result, file.type);
            }
        };
    ...
    return (
        <div className="chat-pane" onClick={dismissMenu}>
            ...
            {/* Compose — label+htmlFor, no programmatic click */}
